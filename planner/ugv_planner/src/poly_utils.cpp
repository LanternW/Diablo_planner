#include <ugv_planner/poly_utils.h>
...
pair<ugv_planner::PolyhedronMSG, decomp_cvx_space::Polytope> polyhedronGenerator::polyHrep2Utils(const pair<MatrixXd, VectorXd> & hrep, const MatrixXd & cur_vertices, bool is_degenarate)
{   
    MatrixXd vertices = getVerticesPlane(hrep, cur_vertices);

    ugv_planner::PolyhedronMSG msg;
...
int polyhedronGenerator::corridorInsertGeneration( const vector<Vector3d> & coordSet, ugv_planner::PolyhedronArrayMSG & poly_array_msg)
{   
    ...
    for ( int pts_cnt = 0; pts_cnt < (int)coordSet.size(); pts_cnt ++ )
    {
        ...
        {   
            ...
            if( poly_cdd.setVertices(vertices) && poly_cdd.lastErrorMessage() == "*No Error found.\n" )
            {
                ...
                ugv_planner::PolyhedronMSG poly_msg = poly_pair.first;
...
int polyhedronGenerator::corridorIncreGeneration( const vector<Vector3d> & coordSet, ugv_planner::PolyhedronArrayMSG & poly_array_msg)
{   
    if( !has_map ) return 0;
...
void polyhedronGenerator::corridorGeneration(const vector<Vector3d> & gridPath, ugv_planner::PolyhedronArrayMSG & poly_array_msg)
{   
    if( !has_map ) return;

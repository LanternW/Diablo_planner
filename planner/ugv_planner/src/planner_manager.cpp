...
namespace ugv_planner
{
  ...
  void UGVPlannerManager::generateFinalMincoTraj(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
  {
    ...
    if (minco_traj_optimizer -> generate_traj(initS, waypoints, pieceN, final_trajectory))
    {
      cout << "[minco optimizer]: optimization success." << endl;
      PublishTraj();
      vis_render.visPolynomialTrajectory(final_trajectory);
    }
  ...
  void UGVPlannerManager::trajPlanning(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
  {
     ...
     generateFinalMincoTraj(start_pt, end_pt);
  }
